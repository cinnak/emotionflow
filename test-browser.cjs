// Simple browser test using console capture
const fs = require('fs');
const path = require('path');

console.log('=== Creating Standalone Browser Test ===\n');

// Read the app.js and extract just the particle system
const appJs = fs.readFileSync(path.join(__dirname, 'app.js'), 'utf8');

// Find the Particle class and EmotionalParticleSystem
const particleClassStart = appJs.indexOf('class Particle {');
const particleSystemStart = appJs.indexOf('class EmotionalParticleSystem {');
const particleSystemEnd = appJs.indexOf('\n}\n', particleSystemStart + 500) + 2;

const particleClass = appJs.substring(particleClassStart, particleSystemStart);
const particleSystem = appJs.substring(particleSystemStart, particleSystemEnd + 100);

console.log('Particle Class:', particleClass.substring(0, 200) + '...');
console.log('\nParticle System:', particleSystem.substring(0, 200) + '...');

// Now create a standalone test file that mimics what the real app does
...
fs.writeFileSync(path.join(__dirname, 'test-particle-system.html'), testHtml);
console.log('\nCreated test-particle-system.html');
console.log('Open this file in a browser to test the particle system directly.');
